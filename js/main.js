...
var resizePizzas = function(size) {
  ...
  // Delete the function determineDx. It is not necessary to call it, since we will change the pizza size inside the changePizzaSizes
...
function updatePositions() {	
  ...
  //Use getElementByClassName because it is faster than the querySelectorAll
 ...
 //var items = document.querySelectorAll('.mover');
  ...
  // phase will only print 5 different numbers. So we create an array where we put the 5 numbers (0, 1, 2, 3, 4) 
  // we also put the phase variable outside the  loop
  var phase = [];  
  for (var i = 0; i < 5; i++) {
  ...
  for (var x = 0; x <lengthItems; x++) {    
    items[x].style.left = items[x].basicLeft + 100 * phase[x % 5] + 'px';
  }
...
document.addEventListener('DOMContentLoaded', function() {  
  var cols = 8;
  var s = 256;  
  var elem; // i move the variable elem outside the loop, to prevent creating it in every time running the loop
  var movingPizzas = document.getElementById('movingPizzas1'); // change from querySelector to getElementById for faster results, and put it outside the loop
  
  /* To calculate the total number of pizzas that should show in the screen, and so create only as much needed, we calculate the screen height,
  then  divide it by s, to know the number of rows, and then round it to an integer. Finally multiply the rows bt cols */
  var screenHeight = window.screen.height;
  var rows = Math.floor(screenHeight / s);
  var numberPizzas = rows * cols;
  //console.log(numberPizzas);
    
  // No need to create 200 pizzas. Just put the exact number of pizzas that will be shown in the screen.
